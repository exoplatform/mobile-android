...
public class AccountSwitcherTest extends ExoActivityTestUtils<AccountSwitcherActivity> {
  ...
  public void verifyDefaultLayout() {
    ...
    String titleText = activity.getTitle().toString();
  ...
  // Removed this test since we don't use the dialog theme anymore
  ...
  public void shouldDismissFragmentAndActivityWhenCurrentAccountIsSelected() {
    ...
    assertThat("1st account should be selected", oldA.accountName, equalTo(TEST_SERVER_NAME + " 1"));

    // Simulate a click on the item at position 0
    shadowOf(accountListView).performItemClick(0);
    ...
    assertThat("1st account should be selected", newA.accountName, equalTo(TEST_SERVER_NAME + " 1"));
  }
  ...
  public void shouldSignOutCurrentAccountAndSignInSelectedAccount() {
    Context ctx = RuntimeEnvironment.application.getApplicationContext();
    create();
    FakeHttp.addHttpResponseRule(getMatcherForRequest(REQ_PLATFORM_INFO_USER_2),
                                 getResponseOKForRequest(REQ_PLATFORM_INFO_USER_2));
    FakeHttp.addHttpResponseRule(getMatcherForRequest(REQ_JCR_USER_2), getResponseOKForRequest(REQ_JCR_USER_2));
    init();
    ...
    shadowOf(accountListView).performItemClick(1);
    ...
    ExoAccount newA = getCurrentAccount(ctx);
    assertThat("2nd account should be selected", newA.accountName, equalTo(TEST_SERVER_NAME + " 2"));
  }
  ...
  public void shouldSignOutAndRedirectToLoginScreenWhenSwitchingFails() {
    create();
    FakeHttp.addHttpResponseRule(getMatcherForRequest(REQ_PLATFORM_INFO), getResponseFailedWithStatus(404));
    FakeHttp.addHttpResponseRule(getMatcherForRequest(REQ_JCR_USER_2), getResponseFailedWithStatus(404));
    init();
    ...
    assertNotNull("There should be a warning Dialog after a failed account switch", warningDialog);
