...
public class LoginActivityTest extends ExoActivityTestUtils<LoginActivity> {
    ...
    public void shouldDisplayPanelWhenTappingButton() {
        ...
        Robolectric.clickOn(serverBtn); // should hide account, display server
                                        // panel and select button when clicking
                                        // the server button
        ...
        Robolectric.clickOn(accountBtn); // should hide server, display account
                                         // panel and select button when
    ...
    public void shouldPopulateServerAndUsernameWhenStartingFromUrl() {
        ...
        Uri uri = new Uri.Builder().scheme("exomobile"). // exomobile://
                                   encodedAuthority(ExoConstants.EXO_URL_USERNAME + "="
                                           + TEST_USER_NAME)
                                   . // username=testuser
                                   appendQueryParameter(ExoConstants.EXO_URL_SERVER,
                                                        TEST_SERVER_URL)
        ...
        user = (EditText) activity.findViewById(R.id.EditText_UserName);
        assertThat(user).containsText(TEST_USER_NAME); // username text field
                                                       // should contain the
                                                       // value passed in the
                                                       // URL
        ...
        org.junit.Assert.assertThat(srv.serverUrl, equalTo(TEST_SERVER_URL)); // server
                                                                              // URL
                                                                              // should
                                                                              // be
                                                                              // that
