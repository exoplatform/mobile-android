...
public class LaunchUtils {
    ...
    private void setServerList() {
        ArrayList<ServerObjInfo> _serverList = ServerConfigurationUtils.getServerListFromFile(mContext,
                                                                                              ExoConstants.EXO_SERVER_SETTING_FILE);
        ServerSettingHelper.getInstance().setServerInfoList(_serverList);

        int selectedServerIdx = Integer.parseInt(mSharedPreference.getString(ExoConstants.EXO_PRF_DOMAIN_INDEX,
                                                                             "-1"));
        mSetting.setDomainIndex(String.valueOf(selectedServerIdx));
        mSetting.setCurrentServer((selectedServerIdx == -1 || selectedServerIdx >= _serverList.size()) ? null
    ...
    private void setOldServerList(String oldConfigFile) {
        ArrayList<ServerObjInfo> _serverList = ServerConfigurationUtils.getServerListFromOldConfigFile(oldConfigFile);
        ServerSettingHelper.getInstance().setServerInfoList(_serverList);
    ...
    private void setAppVersion() {
        try {
            String appVer = mContext.getPackageManager().getPackageInfo(mContext.getPackageName(),
                                                                        0).versionName;
    ...
    private void setLocalize() {
        ...
        if (strLocalize.equals("")) {
            String[] currentLanguagesSupported = new String[] { ExoConstants.ENGLISH_LOCALIZATION,
                    ExoConstants.FRENCH_LOCALIZATION, ExoConstants.GERMAN_LOCALIZATION,
