...
public class ExoConnectionUtils {
	...
	private static String splitLines(String string) {
		...
		for (int i = 0; i < string.length(); i += splitLinesAt) {
			...
			lineBuffer.append(string.substring(i,
		...
		try {
			...
			if (entity != null) {
				...
				if (_strFirstLoginContent == null) {
					...
					if (_strFirstLoginContent
							.contains("Sign in failed. Wrong username or password.")) {
						return ExoConstants.LOGIN_NO;
					} else if (_strFirstLoginContent
							.contains("error', '/main?url")) {
	...
	public static String authorizationHeader(String username, String password) {
		String s = "Basic ";
		String strAuthor = s
				+ stringEncodedWithBase64(username + ":" + password);
		return strAuthor.substring(0, strAuthor.length() - 2);
	}

	public static HttpContext createHttpContext() {
		HttpContext context = new BasicHttpContext();
		context.setAttribute(ClientContext.AUTHSCHEME_REGISTRY, AccountSetting
				.getInstance().getAuthScope());
		context.setAttribute(ClientContext.COOKIESPEC_REGISTRY, _sessionCookies);
		context.setAttribute(ClientContext.CREDS_PROVIDER, AccountSetting
	...
	public static InputStream sendRequestWithAuthorization(String urlStr) {
		...
		try {
			...
			con.setRequestProperty("Authorization",
					authorizationHeader(strUserName, strPassword));
	...
	public static InputStream sendRequestWithoutAuthen(String strUrlRequest) {
		...
		try {
			...
			DefaultHttpClient httpClientPlf = new DefaultHttpClient(
					httpParameters);
			HttpGet httpGet = new HttpGet(strUrlRequest);
	...
	public static void createAuthorization(String url, int port,
	...
	public static boolean checkPLFVersion() {
		try {
			String versionUrl = SocialActivityUtil.getDomain()
					+ ExoConstants.DOMAIN_SUFFIX_VERSION;
			String result = getPLFStream(versionUrl);
			JSONObject json = (JSONObject) JSONValue.parse(result);
			String isComplicant = json.get(ExoConstants.IS_MOBILE_COMPLIANT)
					.toString();
			if (isComplicant.equalsIgnoreCase("true")) {
				String editionObject = json.get(ExoConstants.PLATFORM_EDITION)
						.toString();
				ServerSettingHelper.getInstance().setServerEdition(
						editionObject);
				String verObject = json.get(ExoConstants.PLATFORM_VERSION)
						.toString();
