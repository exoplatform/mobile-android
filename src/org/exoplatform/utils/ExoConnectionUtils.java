...
public class ExoConnectionUtils {
    ...
    public static final String[]    WRONG_CLOUD_URLS           = new String[] {
            "http://exoplatform.net", "http://wks-acc.exoplatform.org",
    ...
    public static HttpResponse requestTenantForEmail(String email) throws IOException {
        return getRequestResponse(HTTP + EXO_CLOUD_WS_DOMAIN + SERVICE_BASE_URL + "/usermailinfo/"
                + email);
    }
    ...
    public static boolean requestAccountExistsForUser(String user, String tenant) {
        String url = HTTP + EXO_CLOUD_WS_DOMAIN + SERVICE_BASE_URL + "/isuserexist/" + tenant + "/"
                + user;
        try {
    ...
    public static boolean urlHasWrongTenant(String url) {
        return Arrays.asList(WRONG_CLOUD_URLS)
                     .contains(!url.startsWith(ExoConnectionUtils.HTTP) ? ExoConnectionUtils.HTTP
                             + url : url);
    }
