...
public class SectionListView extends ListView implements OnScrollListener {
    ...
    /**
     * A flag that indicates whether the user has scrolled to reach the current
    ...
    protected final void commonInitialisation() {
        ...
        autoLoadProgress.setLayoutParams(new ListView.LayoutParams(LayoutParams.WRAP_CONTENT,
                                                                   LayoutParams.WRAP_CONTENT));
        autoLoadProgress.setPadding(0, 0, 0, 10); // 0px left, top and right,
                                                  // 10px bottom
    ...
    public void onScroll(final AbsListView view,
        ...
        if (hasScrolled && totalItemCount > 0
                && firstVisibleItem + visibleItemCount == totalItemCount) {
            autoLoadProgress.setVisibility(View.VISIBLE);
            parentFragment.onLoadMore(ExoConstants.NUMBER_OF_ACTIVITY,
                                      totalItemCount - 1,
                                      firstVisibleItem);
            // set back to false to avoid multiple calls to the onLoadMore(...)
            // method
            hasScrolled = false;
        }
    }
