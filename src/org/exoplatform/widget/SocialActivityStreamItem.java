...
public class SocialActivityStreamItem extends LinearLayout {
	...
	public SocialActivityStreamItem(Context context, SocialActivityInfo info,
			boolean is) {
		super(context);
		mContext = context;
		activityInfo = info;
		...
		imageViewAvatar = (RoundedImageView) view
		...
		textViewCommnet = (TextView) view
				.findViewById(R.id.activity_comment_view);
	...
	private void initCommonInfo() {
		...
		textViewTime.setText(SocialActivityUtil
				.getPostedTimeString(activityInfo.getPostedTime()));
		buttonComment.setText("" + activityInfo.getCommentNumber());
		buttonLike.setText("" + activityInfo.getLikeNumber());
		int imageId = SocialActivityUtil.getActivityTypeId(activityInfo
				.getType());
		...
		case 6:
			...
			try {
				String commentStr = new String(
						templateComment.getBytes("ISO-8859-1"), "UTF-8");
	...
	private void setCaledarContent(TextView textView) {
		try {
			...
			caledarBuffer.append(LocalizationHelper.getInstance().getString(
					"CalendarStart"));
			caledarBuffer.append(" ");
			String startTime = activityInfo.templateParams
					.get("EventStartTime");
			...
			if (isDetail) {
				attachImage.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						SocialDetailHelper.getInstance().setAttachedImageUrl(
								url);
