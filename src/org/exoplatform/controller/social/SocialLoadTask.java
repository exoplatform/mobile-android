...
public class SocialLoadTask extends
		UserTask<Integer, Void, ArrayList<SocialActivityInfo>> {
...
//		progressItem = new ProgressItem(loadingData);
	...
	public ArrayList<SocialActivityInfo> doInBackground(Integer... params) {
		...
		try {
			...
			ActivityService<RestActivity> activityService = SocialServiceHelper
					.getInstance().getActivityService();
			IdentityService<?> identityService = SocialServiceHelper
					.getInstance().getIdentityService();
			RestIdentity identity;
			identity = (RestIdentity) identityService.get(SocialServiceHelper
					.getInstance().getUserId());
			QueryParams queryParams = new QueryParamsImpl();
			queryParams.append(QueryParams.NUMBER_OF_LIKES_PARAM
					.setValue(ExoConstants.NUMBER_OF_LIKES_PARAM));
			queryParams.append(QueryParams.NUMBER_OF_COMMENTS_PARAM
					.setValue(ExoConstants.NUMBER_OF_COMMENTS_PARAM));
			queryParams
					.append(QueryParams.POSTER_IDENTITY_PARAM.setValue(true));
			RealtimeListAccess<RestActivity> list = activityService
					.getFeedActivityStream(identity, queryParams);
			ArrayList<RestActivity> activityList = (ArrayList<RestActivity>) list
					.loadAsList(0, loadSize);
	...
	public void onPostExecute(ArrayList<SocialActivityInfo> result) {
		if (result != null) {
			...
			WarningDialog dialog = new WarningDialog(mContext, titleString,
					contentString, okString);
			dialog.show();
		}
		_progressDialog.dismiss();
	...
	private class SocialWaitingDialog extends WaitingDialog {
		...
		public SocialWaitingDialog(Context context, String titleString,
