...
public class SelectedImageActivity extends MyActionBar implements
		OnClickListener {
	...
	private void onLoad(int id) {
		if (ExoConnectionUtils.isNetworkAvailableExt(this)) {
			if (mLoadTask == null
					|| mLoadTask.getStatus() == DisplayImageTask.Status.FINISHED) {
				mLoadTask = (DisplayImageTask) new DisplayImageTask()
						.execute(id);
			}
		} else {
	...
	private void onCancelLoad() {
		if (mLoadTask != null
				&& mLoadTask.getStatus() == DisplayImageTask.Status.RUNNING) {
	...
	private void backToGallery() {
		...
		if (DocumentActivity._documentActivityInstance != null) {
			DocumentActivity._documentActivityInstance.startActivityForResult(
					intent, ExoConstants.REQUEST_ADD_PHOTO);
	...
	private void onChangeLanguage() {
		...
		loadingData = bundle.getString("LoadingImage");
		title = bundle.getString("SelectedPhoto");
	...
	private class DisplayImageTask extends UserTask<Integer, Void, Bitmap> {
		...
		public void onPreExecute() {
			_progressDialog = new SeclectImageWaitingDialog(
					SelectedImageActivity.this, null, loadingData);
		...
		public Bitmap doInBackground(Integer... params) {
			...
			if (modeId == SELECT_MODE) {
				...
				if (host.equals("media")) {
					filePath = PhotoUtils.getFileFromUri(mImageUri,
					...
					buffer.append(host);
					buffer.append(path);
					Log.i("PHOTO_PICKER", "Image mImageUri getHost(): "
							+ buffer.toString());
					String fileName = mImageUri.getLastPathSegment();
					filePath = PhotoUtils.downloadFile(buffer.toString(),
							fileName);
				}

			} else
				filePath = getIntent().getStringExtra(
						ExoConstants.SELECTED_IMAGE_EXTRA);
			try {
				Log.i("PHOTO_PICKER", "Image File Path: " + filePath);
				file = new File(filePath);
				return PhotoUtils.shrinkBitmap(filePath, SCALE_WIDTH,
						SCALE_HEIGHT);
	...
	private class SeclectImageWaitingDialog extends WaitingDialog {
		...
		public SeclectImageWaitingDialog(Context context, String titleString,
