...
public class AccountSwitcherProxy implements LoginProxy.ProxyListener {
  ...
  public static final String      TAG               = AccountSwitcherProxy.class.getName();
  ...
  public void switchToAccount(ExoAccount account) {
    ...
    } else if (account.username == null || account.username.isEmpty() || account.password == null || account.password.isEmpty()
  ...
  public interface AccountSwitcherListener {
    ...
    /**
     * Called when the switcher proxy detected that the password was missing for
     * the account
     * 
     * @param account the account whose password is missing
     */
    void onMissingPassword(ExoAccount account);
     ...
     */
    void onAccountInvalid(ExoAccount account);
  }
