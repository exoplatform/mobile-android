...
public class SettingUtils {
  ...
  public static void setLocale(Context mContext, String localize) {
    ...
    SharedPreferences.Editor editor = mContext.getSharedPreferences(ExoConstants.EXO_PREFERENCE, 0).edit();
  ...
  public static void modifySharedPerf(Context context) {
    // Modify pref
    SharedPreferences.Editor editor = context.getSharedPreferences(ExoConstants.EXO_PREFERENCE, 0).edit();
    editor.putString(ExoConstants.EXO_PRF_DOMAIN_INDEX, AccountSetting.getInstance().getDomainIndex());
    /*
     * disable saving social filter when login with difference account and clear
